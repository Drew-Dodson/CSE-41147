...
func mealEaten(mealName: String) -> Meal? {
    if let foundMeal = meals[mealName] {
        return foundMeal
    } else {
        return nil
    }
...
//Prob better to unwrap safely but for the sake of brevity we will just leave it like this
print(mealEaten(mealName: "Breakfast"))
print(mealEaten(mealName: "Lunch"))

if let myBreakfast = mealEaten(mealName: "Breakfast") {
    print("My breakfast was the following: ")
    for food in myBreakfast.food {
        print(food)
    }
    print("for a total of \(myBreakfast.calories) calories.")
}
/*:
 ...
 */
// Skip defining a let constant to be used multiple times as this is just a single search and not committing data
//let defaults = UserDefaults.standard

if let savedData = UserDefaults.standard.dictionary(forKey: "mealLog") {
    print(savedData)
} else {
    //value is nil. Good idea to start a new save work flow to log a new meal
}
